...
q_model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=0.1), loss=tf.losses.huber)

q_model.summary()

replay_memory = memory(10000)
...
for i in range(ITERATIONS):
    ...
    while not done:
        ...
        action = epsilon_greedy(epsilon, 4, q_model, state1)
